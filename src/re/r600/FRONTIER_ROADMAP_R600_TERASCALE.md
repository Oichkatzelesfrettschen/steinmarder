...
| Rusticl OpenCL | **8/8 custom tests PASS** (scale, add, mad, sqrt, int, abs, sin, copy) — all sequential in same context, 3/3 stability. RAT readback bug #15 FIXED. Sequential dispatch bug #2 FIXED (256-byte CB alignment in context.rs:479). | RUSTICL_BASELINE.md |
...
| Rusticl conformance | **8/8 custom tests PASS** (scale, add, mad, sqrt, int, abs, sin, copy). Sequential dispatch bug #2 fixed: 256-byte CB alignment in context.rs:479 (ALU_CONST_CACHE addr>>8 truncation). 3/3 stability runs. | RUSTICL_BASELINE.md |
| Sequential dispatch bug #2 | **FIXED** — 256-byte CB alignment in context.rs:479. Evergreen ALU_CONST_CACHE uses addr>>8; 128-byte alignment truncated to wrong address. Diagnostic: strace (9 CS ioctls OK), dmesg (CS rejection), pool_bo dump (input correct), CB0_AT_DISPATCH trace (384→512) | context.rs |
| DRM CS tracing | GFX CS events fire in rcs0 kernel thread, not submitting process. Use strace/bpftrace at syscall level instead of trace-cmd for per-process CS tracking | radeon DRM |
