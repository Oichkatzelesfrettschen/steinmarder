...
int main(int argc, char **argv) {
    ...
    log_size = sizeof(log);
    CHECK_OPTIX(optixProgramGroupCreate(optix_ctx, &raygen_desc, 1, &program_group_options, log, &log_size, &raygen_pg));
    log_size = sizeof(log);
    CHECK_OPTIX(optixProgramGroupCreate(optix_ctx, &miss_desc, 1, &program_group_options, log, &log_size, &miss_pg));
    log_size = sizeof(log);
    CHECK_OPTIX(optixProgramGroupCreate(optix_ctx, &hit_desc, 1, &program_group_options, log, &log_size, &hit_pg));
    ...
    uint32_t css = 0;
    CHECK_OPTIX(optixUtilComputeStackSizes(&stack_sizes, 1, 0, 0, &dss_trav, &dss_state, &css));
    ...
    printf("optix_real_pipeline_result=0x%08x\n", result);
    if (result != 0x100u) {
        fprintf(stderr, "unexpected OptiX payload result: 0x%08x\n", result);
        return 2;
    }
