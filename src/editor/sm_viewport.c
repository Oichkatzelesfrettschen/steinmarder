...
int main(void)
{
    ...
    InitWindow(screenWidth, screenHeight, "steinmarder Realtime Viewport");
    SetTargetFPS(60);

    // Initialize GPU rendering
    int gpu_enabled = sm_gpu_init(screenWidth, screenHeight);
    if(!gpu_enabled) {
    ...
    while (!WindowShouldClose())
    {
        ...
        if(use_gpu_render && gpu_enabled) {
            ...
            if(sm_gpu_render_frame(cam_pos, cam_dir, cam_up, cam.fovy) == 0) {
                unsigned char* fb = sm_gpu_get_framebuffer();
                if(fb) {
        ...
        DrawText("steinmarder Viewport (ALT+LMB orbit, Wheel zoom)", 10, 35, 20, RAYWHITE);
