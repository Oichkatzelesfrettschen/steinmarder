 ...
 * computed via Slater-type orbitals solving the Schrödinger equation.
...
/* Approximate atomic radii in Å (van der Waals or metallic) */
static float atom_radius_A(int Z) {
    /* Selected elements relevant to sion */
    switch (Z) {
        case  1: return 1.20f;  /* H */
        case  2: return 1.40f;  /* He (1s² closed shell) */
 ...
 * to solve the Schrödinger equation for each orbital.
...
static float slater_z_eff(int Z, int n_shell) {
    /* Simplified Slater's rules — matches quantum_volume.c implementation */
...
/* ═══════════════════════ ORBITAL DENSITY ═══════════════════════
 * Compute electron density |ψ|² at a point for a given element.
 * Uses simplified radial Slater-type orbitals:
 *   ψ_STO ∝ r^(n-1) × exp(-Z_eff × r / n)
...
static float orbital_density_at(int Z, float r_A) {
    /* QCD quarks (Z=-1): point-like color charge — Gaussian blob */
    if (Z == -1) {
        float sigma = 0.6f;  /* ~0.6 Å ≈ 0.6 fm for quark charge radius */
        float r2 = r_A * r_A;
        return 5.0f * expf(-r2 / (2.0f * sigma * sigma));
    }

    /* Convert Å to Bohr radii (1 a₀ = 0.529 Å) */
    float r = r_A / 0.529f;  /* r in a₀ */
    ...
    /* This is simplified — the GPU shader does the full computation */
    ...
    for (int s = 0; s < max_shell; s++) {
        ...
        /* STO radial: R(r) ∝ r^(n-1) × exp(-ζr) where ζ = Z_eff/n */
        float zeta = zeff / (float)n;
        float rn = powf(r, (float)(n - 1));
        float radial = rn * expf(-zeta * r);

        /* Normalization (approximate — fine for visualization) */
...
static void setup_single_sion(Atomicsion *af) {
    /* Scene: Single U-235 sion event at atomic scale
     *
     * Layout:
     *   - U-235 atom at origin (with full electron cloud, radius ~1.96 Å)
     *   - Nearby graphite C atom (moderator context) at -8 Å
     *   - Water molecule (H₂O) for context at +8 Å
     *   - Incoming thermal neutron from -20 Å, traveling +X
     *
     * The neutron approaches, gets absorbed, forms U-236* compound nucleus,
     * which deforms and sions into Ba-141 + Kr-92 + 3 prompt neutrons
     * + prompt gammas.
     */
    af->scene = AF_SCENE_SINGLE_SION;
    ...
    af->generation = 0;
    af->boxHalf = 25.0f;  /* ±25 Å — enough for one atom + context */
    ...
    af->atoms[af->num_atoms++] = make_atom(1, 1, 10.96f, 0.76f, 0.0f);  /* O-H bond ≈ 0.96 Å */
    af->atoms[af->num_atoms++] = make_atom(1, 1, 10.24f, -0.93f, 0.0f); /* 104.5° angle */

    /* Incoming thermal neutron — starts far left, travels +X */
    float vn = 12.0f;  /* Å/s display speed (slowed for human perception) */
    ...
    printf("[AF] Scene: Single U-235 sion Event\n");
    printf("[AF]   U-235 atom at origin (Z=92, [Rn]5f³6d¹7s²)\n");
    printf("[AF]   Thermal neutron: E=%.1f meV, λ=%.2f Å\n",
           AF_THERMAL_ENERGY_EV * 1000.0f, AF_THERMAL_WAVELENGTH_A);
    printf("[AF]   σ_sion = %.0f barns, σ_capture = %.0f barns\n",
           AF_U235_SION_XS_BARNS, AF_U235_CAPTURE_XS_BARNS);
}

static void setup_chain_reaction(Atomicsion *af) {
    /* Scene: Multiple U-235 atoms in a lattice — chain reaction
     *
     * UO₂ fuel pellet cross-section: U atoms spaced ~3.87 Å apart
     * in the fluorite crystal structure (Fm3̄m, a = 5.47 Å).
    ...
    /* UO₂ fluorite lattice: U at FCC positions, O in tetrahedral holes
     * Lattice constant a = 5.47 Å
     * U positions (FCC): (0,0,0), (a/2,a/2,0), etc.
     * We'll place a 2×2×2 arrangement ≈ 8 U atoms with O between them */
    float a = 5.47f;  /* UO₂ lattice constant */
    ...
    printf("[AF] Scene: Chain Reaction in UO₂ lattice\n");
    printf("[AF]   %d U-235 atoms in fluorite structure (a=5.47 Å)\n", n_u);
    printf("[AF]   σ_f = %.0f barns × ν̄ = 2.43 → criticality\n",
           AF_U235_SION_XS_BARNS);
}

static void setup_xenon_poison(Atomicsion *af) {
    /* Scene: Xe-135 poisoning
     *
     * Shows the decay chain I-135 → Xe-135 → neutron absorption.
     * Xe-135 has the LARGEST thermal neutron cross section of any
     * known nuclide: σ_a = 2.65 × 10⁶ barns.
     *
    ...
    /* Neutrons traveling through — some will be absorbed by Xe */
    for (int i = 0; i < 4; i++) {
        float dx2, dy2, dz2;
    ...
    printf("[AF]   σ_a(Xe-135) = 2.65×10⁶ barns (largest known!)\n");
    printf("[AF]   I-135 → Xe-135 (t½=6.57h) → neutron absorber\n");
}

static void setup_moderation(Atomicsion *af) {
    /* Scene: Neutron moderation — fast neutron scattering off C/H₂O
     *
     ...
     *     ξ_H = 1.0 (hydrogen — most efficient moderator)
     *     ξ_C = 0.158 (carbon — 114 collisions to thermalize)
    ...
    float c_spacing = 3.35f;  /* graphite interlayer spacing in Å */
    ...
    /* One fast neutron (2 MeV) — will scatter and slow down */
    af->neutrons[af->num_neutrons++] = make_neutron(
        -22.0f, 0.0f, 0.0f,
    ...
    printf("[AF]   Fast neutron: E=2.0 MeV, λ=%.4f Å\n",
           neutron_wavelength_A(AF_FAST_ENERGY_EV));
    printf("[AF]   Target: %d atoms (graphite + water)\n", af->num_atoms);
    printf("[AF]   ξ_C = 0.158 (≈114 collisions to thermalize)\n");
    printf("[AF]   ξ_H = 1.000 (≈18 collisions to thermalize)\n");
}

static void setup_plutonium_sion(Atomicsion *af) {
    /* Scene: Plutonium-239 sion in MOX fuel
     *
     * Pu-239 is the SECOND sile nucleus (after U-235):
     *   σ_f = 747.4 barns (28% HIGHER than U-235!)
     *   ν̄ = 2.88 neutrons per sion (19% more!)
     *   E_sion = 210 MeV
     *
     * Electron configuration: [Rn] 5f⁶ 7s²
     *   94 electrons — 6 in the f-shell (vs 3 for U-235)
     *   Pu has 6 allotropic crystal phases (!!)
     *   α-emitter: t½ = 24,110 years → warm to the touch
     *
     * In a reactor, Pu-239 is bred from U-238 + n → Np-239 → Pu-239
     * MOX fuel (Mixed OXide) contains ~7% PuO₂ + 93% UO₂
     * At Chernobyl Unit 4, ~1.8% of energy came from Pu sion
     *
     * Layout: MOX fuel cluster — Pu and U atoms in PuO₂/UO₂ lattice
     */
    af->scene = AF_SCENE_PLUTONIUM_SION;
    af->phase = AF_PHASE_SETUP;
    af->time = 0.0f;
    af->phase_time = 0.0f;
    af->sion_count = 0;
    ...
    /* MOX fuel lattice: fluorite structure (a = 5.44 Å for PuO₂)
    ...
    printf("[AF] Scene: Plutonium-239 MOX Fuel sion\n");
    printf("[AF]   Pu-239 (Z=94): [Rn] 5f⁶ 7s²\n");
    printf("[AF]   σ_f = %.1f barns (vs U-235: %.1f barns)\n",
           AF_PU239_SION_XS_BARNS, AF_U235_SION_XS_BARNS);
    printf("[AF]   ν̄ = %.2f (vs U-235: 2.43)\n", AF_PU239_NU_BAR);
    printf("[AF]   E_sion = %.0f MeV\n", AF_PU239_SION_ENERGY_MEV);
    printf("[AF]   α-emitter: t½ = %.0f years → warm to the touch\n",
           AF_PU239_ALPHA_HALFLIFE_Y);
}

static void setup_qcd_nucleon(Atomicsion *af) {
    /* Scene: QUANTUM CHROMODYNAMICS — Inside a proton + neutron
     *
     * Scale: ~1 femtometer (10⁻¹⁵ m) = 10⁻⁵ Å
     ...
     *
     * Quark properties:
     *   up:      charge +2/3, mass ~2.2 MeV/c², isospin +1/2
     *   down:    charge -1/3, mass ~4.7 MeV/c², isospin -1/2
     *   strange: charge -1/3, mass ~95  MeV/c², strangeness -1
     *   charm:   charge +2/3, mass ~1.27 GeV/c² (virtual only)
     *
     * Strong force: QCD with SU(3) gauge symmetry
     *   3 color charges: Red, Green, Blue
     *   8 gluon types (carry color + anti-color)
     *   Asymptotic freedom: αs → 0 at short distances
     *   Confinement: V(r) = -4αs/(3r) + κr (Cornell potential)
     *     κ ≈ 0.18 GeV² ≈ 0.89 GeV/fm (string tension)
     *   Flux tubes: gluon fields concentrate into tubes ~0.5 fm diameter
     *
     * Sea quarks (virtual pairs from vacuum fluctuations):
     *   uū, dd̄, ss̄ (and rarely cc̄)
     *   These carry ~50% of the proton’s momentum!
     *   The sea is flavor-asymmetric: more d̄ than ū (NuSea experiment)
     *
     * Entity encoding (all use Z=-1):
     *   A=2 → up quark (valence)
     *   A=1 → down quark (valence)
     *   A=9 → gluon field point
     *   A=3 → light sea quark (uū or dd̄)
     *   A=4 → strange sea quark (ss̄)
     *   A=5 → charm sea quark (cc̄) — very rare, heavy
     *
     * Rendered with Z=-1 special atoms (QCD particles)
     * boxHalf maps ~1 fm → display units for nice visualization
     */
    ...
    /* ── Gluon flux tubes (color field lines) ── */
    ...
    /* ── Sea quarks — virtual pairs from vacuum fluctuations ── */
    /* Light sea (uū and dd̄) — inside both nucleons */
    ...
    /* Strange sea quarks (ss̄) — heavier, rarer, but significant!
     * Strange quark mass ~95 MeV >> up/down (~3-5 MeV)
     * Strangeness contribution to proton spin: Δs ≈ -0.03
    ...
    /* Charm sea quarks (cc̄) — very heavy (1.27 GeV), extremely rare
    ...
    printf("[AF] Scene: QCD — Proton + Neutron Interior\n");
    printf("[AF]   Scale: ~1 fm (10⁻¹⁵ m) per nucleon\n");
    ...
    printf("[AF]   Sea quarks: 4 light (uū/dd̄) + 3 strange (ss̄) + 1 charm (cc̄)\n");
    printf("[AF]   Color charges: R, G, B — SU(3)_c gauge symmetry\n");
    printf("[AF]   αs ≈ 0.3 (running coupling at 1 GeV)\n");
    printf("[AF]   String tension: κ ≈ 0.89 GeV/fm\n");
    printf("[AF]   Cornell potential: V(r) = -4αs/(3r) + κr\n");
    printf("[AF]   Total quarks + antiquarks in scene: %d\n", af->num_atoms);
}

static void setup_chernobyl_sequence(Atomicsion *af) {
    /* Scene: Chernobyl Unit 4 — April 26, 1986 — atomic-scale view
     *
     * What happened physically:
     *   1. Operators reduced power to 200 MW (low Xe-135 burnout)
     *   2. Xe-135 poisoned the core, power dropped to ~30 MW
     *   3. Control rods withdrawn to compensate → dangerously low margin
     *   4. Emergency test: coolant pumps trip → steam voids form
     *   5. Positive void coefficient: voids → MORE reactivity → MORE voids
     *   6. Power surged from 30 MW → 30,000 MW in ~4 seconds
     *   7. Prompt criticality → steam explosion → core destruction
     *
     * At the ATOMIC scale: we show a dense UO₂ lattice with:
     ...
     *   - Multiple simultaneous sion events
     *
     * Visual difference from chain reaction:
     *   - Larger lattice (more U atoms — 16 instead of 9)
     ...
     *   - Boxhalf larger (30 Å) for the bigger scene
     */
    ...
    /* Dense 4×4 uranium lattice (16 atoms — supercritical mass) */
    ...
    /* Xe-135 poison atoms — scattered among the fuel */
    int xe_count = 0;
    ...
    /* Multiple neutrons from different directions — supercritical burst */
    float dirs[][3] = {
        {-28.0f, 2.0f, 0.0f},
        { 0.0f, -28.0f, 1.0f},
        { 25.0f, 10.0f, -1.0f}
    ...
    printf("[AF] Scene: Chernobyl Sequence — RBMK-1000 Atomic View\n");
    printf("[AF]   %d U-235 fuel atoms (4×4 lattice, a=5.47 Å)\n", n_u);
    printf("[AF]   %d Xe-135 poison atoms (neutron absorbers)\n", xe_count);
    printf("[AF]   Water coolant molecules (will boil → void → +ρ)\n");
    printf("[AF]   3 incoming neutrons → supercritical burst\n");
    printf("[AF]   Positive void coefficient: Δk/k ≈ +4.7β\n");
    printf("[AF]   Power excursion: 200 MW → 30,000 MW in ~4 s\n");
}

/* ═══════════════════════ D-T FUSION SCENE ═══════════════════════ */

static void setup_fusion(Atomicsion *af) {
    /* Scene: Deuterium-Tritium Fusion
     *
     * The EASIEST fusion reaction — the one ITER and NIF use:
     *   D(²H) + T(³H) → He-4 (3.52 MeV) + n (14.07 MeV)
     *   Total Q = 17.59 MeV
     *
     * At the atomic scale this is a quantum tunneling problem:
     *   • Classical Coulomb barrier height: E_c = e²/(4πε₀·r) ≈ 0.40 MeV
     *   • But fusion occurs at T ~ 10 keV (40× BELOW the barrier!)
     *   • Gamow peak energy E_G ≈ 6.3 keV
     *   • Tunneling probability P ∝ exp(-π√(E_G/E))
     *
     * Electron configuration:
     *   D = ¹H²: 1s¹ (same as hydrogen but with a neutron in nucleus)
     *   T = ¹H³: 1s¹ (same, but with two neutrons — radioactive, t½=12.3 yr)
     *   He-4 = 1s² (closed shell — noble gas, very stable)
     *
     * The D-T cross section peaks at ~100 keV:
     *   σ(peak) ≈ 5 barns  (at E_cm ≈ 64 keV)
     *
     * Layout: D and T atoms approaching each other with thermal velocity
     * When they get close → quantum tunneling event → fusion flash →
    ...
    printf("[AF]   D(²H) + T(³H) → He-4 (3.52 MeV) + n (14.07 MeV)\n");
    printf("[AF]   Q = %.3f MeV — easiest fusion reaction\n", AF_DT_Q_VALUE_MEV);
    ...
    printf("[AF]   Peak σ ≈ %.1f barns at ~100 keV\n", AF_DT_PEAK_XS_BARNS);
    printf("[AF]   Lawson criterion: nτ > 1.5×10²⁰ m⁻³·s\n");
}

/* Helper: trigger D-T fusion event between two atoms */
static void trigger_fusion(Atomicsion *af, int d_idx, int t_idx) {
    ...
    af->sion_count++;  /* reuse as "fusion count" */
    printf("[AF] ★ FUSION! D+T → He-4 (3.52 MeV) + n (14.07 MeV)  Total Q=%.1f MeV\n",
           af->total_energy_MeV);
}

/* ═══════════════════════ BETA DECAY SETUP ═══════════════════════ */

/* Decay chain data: U-238 → Pb-206 (14 steps) */
typedef struct {
    int Z, A;
    int is_alpha;       /* 1=α, 0=β⁻ */
    float energy_MeV;   /* KE of emitted particle */
    const char *name;
} DecayStep;

static const DecayStep g_u238_chain[AF_DECAY_CHAIN_STEPS] = {
    { 92, 238, 1, 4.270f, "U-238"  },  /* → Th-234 */
    { 90, 234, 0, 0.273f, "Th-234" },  /* → Pa-234 */
    { 91, 234, 0, 2.197f, "Pa-234" },  /* → U-234  */
    { 92, 234, 1, 4.859f, "U-234"  },  /* → Th-230 */
    { 90, 230, 1, 4.770f, "Th-230" },  /* → Ra-226 */
    { 88, 226, 1, 4.871f, "Ra-226" },  /* → Rn-222 */
    { 86, 222, 1, 5.590f, "Rn-222" },  /* → Po-218 */
    { 84, 218, 1, 6.115f, "Po-218" },  /* → Pb-214 */
    { 82, 214, 0, 1.024f, "Pb-214" },  /* → Bi-214 */
    { 83, 214, 0, 3.272f, "Bi-214" },  /* → Po-214 */
    { 84, 214, 1, 7.833f, "Po-214" },  /* → Pb-210 */
    { 82, 210, 0, 0.064f, "Pb-210" },  /* → Bi-210 */
    { 83, 210, 0, 1.162f, "Bi-210" },  /* → Po-210 */
    { 84, 210, 1, 5.407f, "Po-210" },  /* → Pb-206 (stable) */
};
...
static void setup_beta_decay(Atomicsion *af) {
    /* Scene: NEUTRON BETA DECAY (Weak Force)
     *
     * n → p + e⁻ + ν̄ₑ  (mediated by W⁻ boson)
     *
     * The weak force is the only force that changes quark flavor.
     * Inside the neutron, a down quark (charge -1/3) emits a virtual
     * W⁻ boson (mass 80.4 GeV!) and becomes an up quark (charge +2/3).
     * The W⁻ then decays into an electron + electron antineutrino.
     *
     * This is Fermi's theory of beta decay (1933):
     *   d → u + W⁻ → u + e⁻ + ν̄ₑ
     *
     * Free neutron half-life: 611 s (10.2 min)  (ucn τ experiment)
     * Q-value: (mₙ - mₚ)c² = 1.2934 MeV
     * Max electron KE: 0.782 MeV (endpoint)
     * W⁻ boson: m = 80.377 GeV, τ ≈ 3×10⁻²⁵ s (virtual here)
     *
     * We show the neutron as 3 quarks (udd), then animate:
     *   1. A down quark glows and emits a W⁻ bubble
     *   2. The down quark transforms to up quark (color change)
     *   3. The W⁻ propagates briefly then decays
    ...
    af->atoms[af->num_atoms++] = d2;  /* down quark 2 — this one will decay */
    ...
    printf("[AF]   n → p + e⁻ + ν̄ₑ  via W⁻ boson\n");
    printf("[AF]   W⁻ mass: %.3f GeV/c²\n", AF_W_BOSON_MASS_GEV);
    ...
    printf("[AF]   d → u + W⁻ → u + e⁻ + ν̄ₑ\n");
}

/* ═══════════════════════ CONFINEMENT BREAK SETUP ═══════════════════════ */

static void setup_confinement(Atomicsion *af) {
    /* Scene: QCD CONFINEMENT BREAK (String Snapping → Hadronization)
     *
     * When you try to separate two quarks, the color flux tube
     * (QCD string) stretches. The potential energy is V(r) = κr
     * where κ ≈ 0.89 GeV/fm (string tension).
     *
     * At ~1 fm separation, the energy stored in the string exceeds
     * 2×mq (enough to create a new quark-antiquark pair). The string
     * SNAPS and a new q-q̄ pair materializes from the vacuum.
     *
     * This is why free quarks have NEVER been observed:
     *   Pull q apart → string stores energy → string breaks →
     *   new q̄q pair → you get two mesons, never a free quark!
     *
     * Animation:
     *   1. Two quarks (q and q̄) connected by a flux tube (meson)
     *   2. External force pulls them apart
     *   3. Flux tube stretches, glows brighter (energy increasing)
     *   4. SNAP! New q-q̄ pair appears at break point
    ...
    printf("[AF]   String tension: κ ≈ 0.89 GeV/fm\n");
    printf("[AF]   Pulling apart → V(r) = κr → string snaps\n");
    printf("[AF]   New qq̄ pair from vacuum → two mesons!\n");
    printf("[AF]   Demonstrates why free quarks don't exist\n");
}

/* ═══════════════════════ DECAY CHAIN SETUP ═══════════════════════ */

static void setup_decay_chain(Atomicsion *af) {
    /* Scene: U-238 → Pb-206 RADIOACTIVE DECAY CHAIN
     *
     ...
     *
     *  Each step: parent atom at center, α or β particle emitted,
    ...
    printf("[AF] Scene: Decay Chain (U-238 → Pb-206)\n");
    printf("[AF]   14 radioactive decays: 8α + 6β⁻\n");
    printf("[AF]   Half-life: %.3f billion years\n", AF_U238_HALFLIFE_GY);
    printf("[AF]   Starting isotope: U-238 (Z=92, A=238)\n");
    printf("[AF]   Final product: Pb-206 (stable)\n");
}

/* ═══════════════════════ BINDING ENERGY SETUP ═══════════════════════ */

static void setup_binding_energy(Atomicsion *af) {
     ...
     *   - Broad peak at Fe-56 (8.79 MeV/nucleon) — most stable nucleus
     *   - Slow decline for heavy nuclei (U-238: 7.57 MeV)
     *
     * THIS is why:
     *   - Light nuclei FUSE (climbing the curve → releases energy)
     *   - Heavy nuclei SION (descending from heavy side → releases energy)
    ...
    printf("[AF]   Light nuclei → fusion gains energy\n");
    printf("[AF]   Heavy nuclei → sion gains energy\n");
    printf("[AF]   Iron-56: the ash of stellar nucleosynthesis\n");
}

/* ═══════════════════════ PHYSICS UPDATE ═══════════════════════ */

static void trigger_sion(Atomicsion *af, int atom_idx) {
    /* The target atom sions!
     *
     * U-235 + n → [U-236]* → Ba-141 + Kr-92 + 3n + γ + 200 MeV
     *
     * sion fragment kinetic energies (from Coulomb repulsion):
     *   E_Ba ≈ 60 MeV → v ≈ 1.1 × 10⁷ m/s
     *   E_Kr ≈ 95 MeV → v ≈ 1.5 × 10⁷ m/s
    ...
    /* Choose sion axis (random direction) */
    float fx, fy, fz;
    af_rand_dir(&fx, &fy, &fz);

    /* Create Ba-141 fragment (heavier → slower) */
    if (af->num_atoms < AF_MAX_ATOMS) {
        AF_Atom ba = make_atom(AF_FRAG1_Z, AF_FRAG1_A, ox, oy, oz);
        ba.state = AF_ATOM_FRAGMENT;
        ba.vx = fx * 8.0f;   /* Å/s display speed */
        ba.vy = fy * 8.0f;
        ba.vz = fz * 8.0f;
        ba.glow = 3.0f;      /* bright sion flash */
        ba.orbital_scale = 0.5f;  /* starts compressed */
        af->atoms[af->num_atoms++] = ba;
    }

    /* Create Kr-92 fragment (lighter → faster, opposite direction) */
    if (af->num_atoms < AF_MAX_ATOMS) {
    ...
    int n_prompt = 2 + (af_rand01() > 0.57f ? 1 : 0);  /* ν̄ = 2.43 */
    for (int i = 0; i < n_prompt && af->num_neutrons < AF_MAX_NEUTRONS; i++) {
        float dx2, dy2, dz2;
        af_rand_dir(&dx2, &dy2, &dz2);
        /* Prompt neutron spectrum: Watt sion spectrum
         * P(E) ∝ exp(-E/a) × sinh(√(bE))  with a=0.988, b=2.249 for U-235
         * Average E ≈ 2 MeV, but ranges from 0.1 to 10 MeV */
    ...
    printf("[AF] ══ SION EVENT #%d ══\n", af->sion_count);
    printf("[AF]   ²³⁵U + n → ¹⁴¹Ba + ⁹²Kr + %dn + γ\n", n_prompt);
    printf("[AF]   Energy released: %.0f MeV (total: %.0f MeV)\n",
           AF_U235_SION_ENERGY_MEV, af->total_energy_MeV);
}

static void scatter_neutron(Atomicsion *af, int n_idx, int atom_idx) {
    /* Elastic scattering: neutron bounces off nucleus
     *
     * Energy transfer per collision (center-of-mass frame):
     *   E_after / E_before = [(A-1)/(A+1)]² (head-on) to 1 (grazing)
     *   Average: E_after = E_before × [(A+1)² - 2] / (A+1)²
     *
     * For C-12: E_after ≈ 0.716 × E_before (28.4% loss per collision)
     * For H-1:  E_after ≈ 0 to E_before (average 50% loss)
     */
    ...
    /* New random direction (isotropic in CM frame → ~isotropic in lab for heavy nuclei) */
    float dx, dy, dz;
    af_rand_dir(&dx, &dy, &dz);
    float speed = sqrtf(n->vx*n->vx + n->vy*n->vy + n->vz*n->vz);
    speed *= sqrtf(frac);  /* slow down proportional to √(E_ratio) */
...
void atomic_sion_update(Atomicsion *af, float dt) {
    ...
    for (int i = 0; i < af->num_neutrons; i++) {
        ...
        for (int j = 0; j < af->num_atoms; j++) {
            ...
            /* Interaction radius: capture radius ~ nuclear radius (fm → Å scale)
             * Nuclear radius ~7 fm = 0.0007 Å, way too small to see.
             * For visualization, use ~1.5 Å as effective capture radius
            ...
            if (dist2 < capture_r * capture_r) {
                ...
                if (a->Z == 92 && a->A == 235 && n->is_thermal) {
                    ...
                    if (af_rand01() < 0.855f) {
                        ...
                        /* Radiative capture: U-235 + n → U-236 + γ */
                        a->Z = 92; a->A = 236;
                    ...
                    /* Pu-239 thermal sion: σ_f/(σ_f+σ_γ) = 747/(747+270) ≈ 73.4%
                     * Higher sion cross section but also higher capture! */
                    if (af_rand01() < 0.734f) {
                        /* sion: Pu-239 + n → Ce-144 + Kr-94 + 3n + γ + 210 MeV */
                        trigger_sion(af, j);
                        ...
                        /* Pu-239 + n → Pu-240 (non-sile!): σ_γ = 270 barns */
                        a->A = 240;
                        a->state = AF_ATOM_EXCITED;
                        a->state_time = 0.0f;
                        a->glow = 1.5f;
                        n->alive = 0;
                        printf("[AF] Pu-239 captured neutron → Pu-240 (non-sile)\n");
                    }
                    ...
                    a->state_time = 0.0f;
                    a->glow = 2.0f;
                    a->A = 136;  /* Xe-135 + n → Xe-136 */
                    n->alive = 0;
                    printf("[AF] Xe-135 absorbed neutron! (σ=2.65M barns)\n");
                } else if (a->Z == 6 || a->Z == 8 || a->Z == 1) {
                    /* Moderator scattering */
                    scatter_neutron(af, i, j);
                    if (n->scattered_count == 1 && a->Z == 1) {
                        printf("[AF] Neutron scattered off %s: E=%.0f eV → %.0f eV\n",
                               a->Z == 1 ? "H" : a->Z == 6 ? "C" : "O",
    ...
    /* ── Update gamma ray photons ── */
    float gamma_speed = 120.0f;  /* Å/s display speed (c = ∞ effectively) */
    ...
    for (int i = 0; i < af->num_atoms; i++) {
        ...
        case AF_ATOM_ABSORBING:
            /* Xe-135 absorbing neutron — glow then settle */
            a->glow *= (1.0f - dt * 1.0f);
            if (a->glow < 0.01f) {
                a->state = AF_ATOM_IDLE;
            }
    ...
    /* ── Xenon scene: gradually reveal Xe atoms (simulates I-135 decay) ── */
    if (af->scene == AF_SCENE_XENON_POISON) {
        ...
        for (int i = 1; i < af->num_atoms && xe_visible > 0; i++) {
            if (af->atoms[i].Z == 54 && !af->atoms[i].visible) {
                ...
                xe_visible--;
                printf("[AF] Xe-135 appeared (I-135 decay → Xe-135)\n");
            }
        }
    ...
    /* ── QCD scene: quarks orbit, gluon field fluctuates ── */
    if (af->scene == AF_SCENE_QCD_NUCLEON) {
        ...
        float orbit_rate = 1.2f; /* rad/s — slow enough to see */
        ...
        for (int i = 0; i < af->num_atoms; i++) {
            ...
            if (a->A == 1 || a->A == 2) {
                /* Valence quark — orbit around parent nucleon center */
                ...
                /* Gluon field point — fluctuate along flux tubes */
                ...
                /* Light sea quark pair — flash in and out (virtual pair creation) */
                ...
                /* Strange sea quark (ss̄) — heavier, shorter-lived fluctuation
                 * Mass ~95 MeV vs ~5 MeV for u/d → shorter virtual lifetime
                 * ΔE·Δt ≈ ℏ → Δt ≈ ℏ/m_s ≈ 0.002 fm/c */
                float phase = sinf(t * 6.0f + (float)i * 2.3f);
                a->visible = (phase > 0.3f) ? 1 : 0;  /* less time visible (heavier) */
                if (a->visible) {
                    a->glow = (phase - 0.3f) * 1.2f;
                    /* Strange quarks orbit tighter (heavier → shorter Compton λ) */
                ...
                /* Charm sea quark (cc̄) — very heavy (1.27 GeV), extremely rare
                 * Appears as brief bright flash, very compact */
                float phase = sinf(t * 8.0f + (float)i * 3.7f);
                a->visible = (phase > 0.6f) ? 1 : 0;  /* barely visible — very heavy */
                if (a->visible) {
    ...
    /* ── D-T Fusion scene: atoms approach → tunnel → fuse ── */
        ...
        for (int i = 0; i < af->num_atoms; i++) {
            ...
            for (int j = i + 1; j < af->num_atoms; j++) {
                ...
                if (dist2 < tunnel_r * tunnel_r) {
                     ...
                     * P ∝ exp(-π√(E_G/E)) — Gamow factor
                     * At ~10 keV: P ≈ 10⁻⁵ (but we force it for visualization) */
                 ...
                 * due to nuclear force — the strong force takes over at ~2 fm) */
                float dist = sqrtf(dist2);
                if (dist < 8.0f && dist > 0.1f) {
                    /* Gentle Coulomb push (e²/r² but dramatically scaled) */
    ...
    /* ── Plutonium alpha decay: Pu-239 → U-235 + α (He-4) ── */
    if (af->scene == AF_SCENE_PLUTONIUM_SION) {
        /* Real t½ = 24,110 years → compress to ~1 event per ~8 seconds for vis */
        ...
        if (alpha_timer > 8.0f) {
            ...
            for (int i = 0; i < af->num_atoms; i++) {
                ...
                if (a->Z == 94 && a->A == 239 && a->visible && a->state == AF_ATOM_IDLE) {
                    /* Alpha decay: Pu-239 → U-235 + He-4 (α) */
                    a->Z = 92;
                    a->A = 235;
                    a->state = AF_ATOM_EXCITED;
                    a->state_time = 0.0f;
                    ...
                    if (af->num_atoms < AF_MAX_ATOMS) {
                        ...
                        alpha.vx = ax * 15.0f;  /* 5.24 MeV → fast for He */
                        alpha.vy = ay * 15.0f;
                        alpha.vz = az * 15.0f;
                        alpha.glow = 2.0f;
                    ...
                    printf("[AF] α-decay: Pu-239 → U-235 + He-4 (%.3f MeV)\n",
    ...
    /* ── Beta Decay scene: animate W boson emission ── */
    if (af->scene == AF_SCENE_BETA_DECAY) {
        float t = af->time;

        /* Phase timing (revised — dramatic beats get more screen time):
         *   0-0.8s:   quarks orbit, building visual recognition
         *   0.8-1.6s: down quark #2 glows hot — about to decay
         *   1.6s:     W⁻ boson ERUPTS from the quark (big golden bubble)
         *   1.6-4.5s: W⁻ expands slowly, pulsing — you see its 80 GeV mass
         *   4.5s:     W⁻ DECAYS → electron + antineutrino shoot out
         *   4.5-5.5s: flavor change visible (d→u, quark recolors)
         *   5.5-18s:  electron spirals, neutrino streaks — long visible tail
         *   18s:      restart
         */
        ...
        if (t > 0.8f && t < 1.6f && af->num_atoms >= 3) {
            float pulse = (t - 0.8f) / 0.8f;  /* 0→1 ramp */
            af->atoms[2].glow = 1.5f + pulse * 3.0f + sinf(pulse * 12.0f) * 0.5f;
        }

        /* Phase 1.6s: emit W⁻ boson (particle type 4) — big dramatic moment */
        if (t > 1.6f && t < 1.7f && af->num_particles == 0) {
            ...
            w.y = af->atoms[2].y;
            w.z = af->atoms[2].z;
            w.vx = 0.3f; w.vy = -0.5f; w.vz = 0.15f;  /* SLOW — it's massive */
            w.energy_MeV = AF_W_BOSON_MASS_GEV * 1000.0f;
            w.birth_time = t;
            w.lifetime = 3.0f;   /* visual: 3 seconds of W⁻ glory */
            w.radius = 1.5f;
            w.alive = 1;
            w.type = 4;  /* W⁻ boson */
            af->particles[af->num_particles++] = w;

            /* Transform the down quark → up quark (flavor change!) */
        ...
        /* W boson pulsing glow while alive (1.6 → 4.5s) */
        ...
        /* Phase 4.5s: W⁻ decays → electron + neutrino (the big payoff) */
        if (t > 4.5f && af->num_particles >= 1) {
            ...
            if (w->alive && w->type == 4) {
                ...
                /* Electron (β⁻) — SLOW so it stays visible longer */
                AF_Particle e;
                memset(&e, 0, sizeof(e));
                ...
                /* Antineutrino (ν̄ₑ) — also slower for visibility */
                AF_Particle nu;
        ...
        for (int i = 0; i < af->num_particles; i++) {
            ...
            p->y += p->vy * dt;
            p->z += p->vz * dt;
            /* Electron spirals (Larmor radius ~0.5 Å in ~1T field) */
            if (p->type == 0) {
            ...
            if (p->type == 4) {
                float age = t - p->birth_time;
                p->radius = 1.5f + age * 1.5f;  /* grows to ~6 Å over 3s */
                /* Pulsing glow */
        ...
        if (t > 18.0f) {
            printf("[AF] Beta decay complete — restarting...\n");
            atomic_sion_setup(af, af->scene);
            return;
        }
    }

    /* ── Confinement Break scene: stretch flux tube → snap → hadronize ── */
    if (af->scene == AF_SCENE_CONFINEMENT) {
        float t = af->time;

        /* Revised timing — much slower, more dramatic:
         *   0-6s:    Quarks slowly separate, gluon string stretches visibly
         *            String tension builds (glow increases, oscillations grow)
         *   6-6.5s:  Critical tension — string glowing white-hot
         *   6.5s:    SNAP! String breaks — massive gamma flash
         *   6.5-7.5s: New q-q̄ pair materializes at break point
         *   7.5-16s: Two new mesons drift apart slowly, glow fading
         *   16s:     Restart
         */

        float stretch_end = 6.0f;    /* string stretches 0 → 6s */
        ...
        for (int i = 2; i < af->num_atoms; i++) {
            ...
            if (!snapped) {
                ...
                float tension_scale = sep / max_sep;  /* 0→1 as string stretches */
        ...
        if (snapped && !conf_pair_created && t > snap_time && t < snap_time + 0.2f) {
            ...
            if (af->num_atoms + 2 <= AF_MAX_ATOMS) {
                /* New antiquark (pairs with left quark → meson 1) */
                ...
                /* New quark (pairs with right antiquark → meson 2) */
        ...
        if (t < stretch_end) {
            ...
            snprintf(af->hud_lines[0], 80, "Two mesons formed — confinement preserved");
        }

        /* Restart after 16s */
        if (t > 16.0f) {
            printf("[AF] Confinement cycle — restarting string...\n");
            atomic_sion_setup(af, af->scene);
            return;
        }
    }

    /* ── Decay Chain scene: step through U-238 → Pb-206 ── */
    if (af->scene == AF_SCENE_DECAY_CHAIN) {
        ...
        if (af->chain_step_time > step_interval && af->chain_step < AF_DECAY_CHAIN_STEPS) {
            ...
            if (ds->is_alpha) {
                ...
                /* Emit α particle — slow enough to see it leave */
                float ax, ay, az;
                af_rand_dir(&ax, &ay, &az);
                if (af->num_atoms < AF_MAX_ATOMS) {
                ...
                /* Emit electron (β⁻) — slow for visibility */
                if (af->num_particles < AF_MAX_PARTICLES) {
                    AF_Particle e;
                    memset(&e, 0, sizeof(e));
                    float ex, ey, ez;
        ...
        if (af->chain_step >= AF_DECAY_CHAIN_STEPS && af->chain_step_time > 4.0f) {
            printf("[AF] Decay chain complete: Pb-206 (stable) — restarting...\n");
            atomic_sion_setup(af, af->scene);
            return;
        }
    }

    /* ── Binding Energy scene: nuclei pulsate, morph visualization ── */
    if (af->scene == AF_SCENE_BINDING_ENERGY) {
        float t = af->time;

        /* Slowly scan A from 2 → 238 and back, highlighting the current nucleus */
    ...
    /* ── Looping: restart when all activity dies down ── */
    ...
    if (af->looping &&
        ...
        if (!any_alive && !any_fragment && af->phase_time > 3.0f) {
            ...
            if (!still_active) {
                printf("[AF] Scene complete — restarting...\n");
                atomic_sion_setup(af, af->scene);
            }
        }
    }
 ...
 *   density = electron density |ψ|² (or neutron/gamma intensity)
 *   signed  = material encoding:
 *     +3.5  = Pu-239 (plutonium)
 *     +3.0  = U-235/U-236 (uranium atom)
 *     +2.5  = Ba/Kr sion fragment (excited)
 *     +2.0  = Xe-135 (poison)
 *     +1.5  = O (oxygen)
 ...
 *     -4.5  = QCD strange quark (ss̄)
 *     -5.0  = QCD charm quark (cc̄)
 *     -5.5  = W boson (weak force carrier)
 *     -6.0  = electron / beta particle (β⁻/β⁺)
 *     -6.5  = neutrino (ν / ν̄)
 *      0.0  = empty
 * ═══════════════════════════════════════════════════════════ */
...
static float af_signed_for_atom(const AF_Atom *a) {
    ...
    if (a->state == AF_ATOM_FRAGMENT) return 2.5f;  /* sion fragment */
    ...
    if (a->Z == -1) {
        if (a->A == 5) return -5.0f;     /* QCD charm quark (cc̄) */
        if (a->A == 4) return -4.5f;     /* QCD strange quark (ss̄) */
...
int atomic_sion_visualize(Atomicsion *af, VkQueue queue) {
    ...
    /* ── Stamp each atom's electron density into the grid ── */
    for (int ai = 0; ai < af->num_atoms; ai++) {
        ...
        /* Atom electron cloud radius (in Å) — larger for heavier atoms */
        float cloud_r = a->radius_A * 3.5f * a->orbital_scale;
        if (a->Z == 92) cloud_r = 8.0f * a->orbital_scale;  /* U has extended orbitals */
        if (a->Z == 94) cloud_r = 8.5f * a->orbital_scale;  /* Pu: slightly larger 5f⁶ */
        if (a->Z == 54) cloud_r = 6.0f * a->orbital_scale;  /* Xe likewise */
        if (a->Z == 2)  cloud_r = 3.0f * a->orbital_scale;  /* He: compact 1s² closed shell */
        ...
        for (int dz = -cr; dz <= cr; dz++) {
            ...
            for (int dy = -cr; dy <= cr; dy++) {
                ...
                for (int dx = -cr; dx <= cr; dx++) {
                    ...
                    /* Additive blending — overlapping atoms combine */
    ...
    for (int ni = 0; ni < af->num_neutrons; ni++) {
        ...
        /* Neutron wave packet size: larger for thermal (longer λ) */
        ...
        for (int dz = -cr; dz <= cr; dz++) {
            ...
            for (int dy = -cr; dy <= cr; dy++) {
                ...
                for (int dx = -cr; dx <= cr; dx++) {
                    ...
                    /* De Broglie wave packet: Gaussian envelope × plane wave
                     * ψ(r) = exp(-r²/2σ²) × cos(2π r / λ)
                     * where σ = packet width, λ = de Broglie wavelength */
    ...
    /* ── Stamp particles (electrons, neutrinos, W bosons, alpha) ── */
    for (int pi = 0; pi < af->num_particles; pi++) {
        ...
        float prad;
        switch (p->type) {
        case 0:  sv = -6.0f; prad = 0.8f; break;   /* electron (β⁻) */
        case 1:  sv = -6.0f; prad = 0.8f; break;   /* positron (β⁺) */
        case 2:  sv =  0.65f; prad = 1.2f; break;  /* alpha → same as He-4 */
        case 3:  sv = -6.5f; prad = 0.5f; break;   /* neutrino (ghost) */
        case 4:  sv = -5.5f; prad = p->radius; break; /* W boson */
        case 5:  sv = -5.5f; prad = p->radius; break; /* W⁺ */
...
int atomic_sion_init(Atomicsion *af,
    ...
    printf("[AF] Atomic sion visualization initialized (grid=%d³)\n", gridDim);
    return 0;
}

void atomic_sion_setup(Atomicsion *af, AF_SceneType scene) {
    /* Reset all entities */
    ...
    switch (scene) {
    case AF_SCENE_SINGLE_SION:    setup_single_sion(af);    break;
    ...
    case AF_SCENE_PLUTONIUM_SION: setup_plutonium_sion(af); break;
    ...
    case AF_SCENE_BINDING_ENERGY:
        ...
        break;
    default:
        setup_single_sion(af);
        break;
    }

    /* Ensure scene type matches the requested scene (setup functions
     * set af->scene to their own type, which is wrong for fallbacks
     * like Chernobyl → chain_reaction). */
    af->scene = scene;
}

void atomic_sion_next_scene(Atomicsion *af) {
    AF_SceneType next = (AF_SceneType)((af->scene + 1) % AF_SCENE_COUNT);
    atomic_sion_setup(af, next);
}

const char *atomic_sion_scene_name(AF_SceneType scene) {
    switch (scene) {
    case AF_SCENE_SINGLE_SION:     return "U-235 sion";
    ...
    case AF_SCENE_PLUTONIUM_SION:  return "Pu-239 MOX sion";
...
const char *atomic_sion_phase_name(AF_Phase phase) {
    switch (phase) {
    ...
    case AF_PHASE_EXPLOSION:      return "sion Products";
...
void atomic_sion_free(Atomicsion *af) {
    VkDevice dev = af->device;
    if (dev) {
        if (af->cmdPool) vkDestroyCommandPool(dev, af->cmdPool, NULL);
    }
    printf("[AF] Atomic sion system freed\n");
    memset(af, 0, sizeof(*af));
}
