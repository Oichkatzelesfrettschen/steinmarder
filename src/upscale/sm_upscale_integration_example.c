/* sm_upscale_integration_example.c — Reference integration of steinmarder Neural Upscale
 *
...
typedef struct MyRenderer {
    ...
    /* Upscale system */
    SmUpscaleCtx    upscale;

    /* Frame tracking */
    uint32_t         frame_index;
    uint32_t         display_w, display_h;

    SmUpscaleQuality quality_preset;
} MyRenderer;
...
VkResult my_init_upscale(MyRenderer* r) {
    ...
    SmUpscaleInitInfo info = {
        ...
        .max_display_h       = 2160,
        .weight_file_path    = "models/sm_upscale_v1.weights",  /* NULL for fallback */
    ...
    VkResult result = sm_upscale_init(&r->upscale, &info);
...
void my_render_frame(MyRenderer* r, VkCommandBuffer cmd) {
    ...
    uint32_t w_lo = (uint32_t)(r->display_w * scale);
    uint32_t h_lo = (uint32_t)(r->display_h * scale);
    SmJitter jitter = sm_upscale_jitter(r->frame_index);

    /* --- Phase D: Fill frame params and execute --- */
    SmUpscaleFrameParams params = {
    ...
    sm_upscale_update_descriptors(&r->upscale, &params);

    /* Record 3-pass upscale pipeline */
    sm_upscale_execute(&r->upscale, cmd, &params);
...
void my_dynamic_resolution_update(MyRenderer* r, float frame_time_ms) {
    ...
    if (frame_time_ms > budget * 1.2f && r->quality_preset < SM_UPSCALE_QUALITY_ULTRA_PERF) {
        /* Over budget by 20%+ — step down quality (lower internal res) */
        r->quality_preset = (SmUpscaleQuality)(r->quality_preset - 1);
        fprintf(stderr, "[DRS] Stepped down to quality %d (%.1fms)\n",
                r->quality_preset, frame_time_ms);
    } else if (frame_time_ms < budget * 0.7f && r->quality_preset < SM_UPSCALE_QUALITY_ULTRA) {
 ...
 *       .commandBufferCount  = 1,
 *       .pCommandBuffers     = &compute_cmd,   // recorded with sm_upscale_execute
 *       .signalSemaphoreCount = 1,
 ...
 * images (color_lo, depth_lo, mv). The current sm_upscale.c assumes
 ...
 *   with open('sm_upscale_v1.weights', 'wb') as f:
