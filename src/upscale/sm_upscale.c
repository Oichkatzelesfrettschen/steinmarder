/* sm_upscale.c — DLSS-class temporal super-resolution (Vulkan compute)
 *
...
static int sm_us_env_int(const char* key, int def) {
    const char* v = getenv(key);
    return (v && v[0]) ? atoi(v) : def;
}

static float sm_us_env_float(const char* key, float def) {
    const char* v = getenv(key);
    return (v && v[0]) ? (float)atof(v) : def;
}

/* Find a memory type index satisfying both type bits and property flags */
static uint32_t sm_find_memory_type(VkPhysicalDevice pd,
                                     uint32_t type_bits,
...
static VkResult sm_create_image_2d(VkDevice dev, VkPhysicalDevice pd,
                                    ...
                                    VkImageView* out_view)
{
    ...
    uint32_t memIdx = sm_find_memory_type(pd, mreq.memoryTypeBits,
...
static void sm_destroy_image(VkDevice dev, const VkAllocationCallbacks* alloc,
...
static VkResult sm_create_shader_module(VkDevice dev,
                                         const VkAllocationCallbacks* alloc,
...
static VkResult sm_create_samplers(SmUpscaleCtx* ctx) {
    VkResult r;
...
static VkResult sm_create_descriptors(SmUpscaleCtx* ctx) {
    VkResult r;
...
static VkResult sm_create_compute_pipeline(VkDevice dev,
                                            const VkAllocationCallbacks* alloc,
                                            VkShaderModule sm,
...
static VkResult sm_create_pipelines(SmUpscaleCtx* ctx) {
    VkResult r;
    VkDevice dev = ctx->device;
    ...
    /* ----- Compute pipelines ----- */
    r = sm_create_compute_pipeline(dev, ctx->alloc, ctx->sm_reproj,
                                    ctx->pl_layout_reproj, &ctx->pipeline_reproj);
    if (r != VK_SUCCESS) return r;

    r = sm_create_compute_pipeline(dev, ctx->alloc, ctx->sm_neural,
    ...
    if (ctx->sm_neural_fallback) {
        r = sm_create_compute_pipeline(dev, ctx->alloc, ctx->sm_neural_fallback,
                                        ctx->pl_layout_neural, &ctx->pipeline_neural_fallback);
        if (r != VK_SUCCESS) return r;
    }

    r = sm_create_compute_pipeline(dev, ctx->alloc, ctx->sm_sharpen,
...
VkResult sm_upscale_load_weights(SmUpscaleCtx* ctx, const char* path) {
    ...
    if (!f) {
        fprintf(stderr, "[sm_upscale] Cannot open weight file: %s\n", path);
        ctx->has_neural_weights = 0;
        return VK_SUCCESS; /* degrade gracefully to fallback */
    }

    /* Read header */
    SmUpscaleWeightHeader hdr;
    if (fread(&hdr, sizeof(hdr), 1, f) != 1) {
        fprintf(stderr, "[sm_upscale] Weight file header read failed\n");
        fclose(f);
        ctx->has_neural_weights = 0;
        return VK_SUCCESS;
    }

    if (hdr.magic != SM_UPSCALE_WEIGHT_MAGIC || hdr.version != SM_UPSCALE_WEIGHT_VER) {
        fprintf(stderr, "[sm_upscale] Weight file magic/version mismatch "
                "(got 0x%08X v%u, expected 0x%08X v%u)\n",
                hdr.magic, hdr.version, SM_UPSCALE_WEIGHT_MAGIC, SM_UPSCALE_WEIGHT_VER);
        fclose(f);
        ctx->has_neural_weights = 0;
        return VK_SUCCESS;
    }

    if (hdr.param_count > SM_UPSCALE_MAX_PARAMS) {
        fprintf(stderr, "[sm_upscale] Weight param_count %u > max %u\n",
                hdr.param_count, SM_UPSCALE_MAX_PARAMS);
        fclose(f);
        ctx->has_neural_weights = 0;
        return VK_SUCCESS;
    }
    ...
    if (fread(data, 1, data_bytes, f) != data_bytes) {
        fprintf(stderr, "[sm_upscale] Weight data read incomplete\n");
        free(data);
        fclose(f);
        ctx->has_neural_weights = 0;
        return VK_SUCCESS;
     ...
     * VkPhysicalDevice is not stored in SmUpscaleCtx.  Until that is added,
     ...
     *
     * TODO(production): store phys_device in SmUpscaleCtx at init, then:
    ...
    fprintf(stderr, "[sm_upscale] WARNING: weight buffer created but memory binding not implemented.\n"
                    "  Neural upscaling is DISABLED until VkPhysicalDevice is plumbed through.\n");
    fprintf(stderr, "[sm_upscale] Loaded %u neural params (%.1f KB FP16)\n",
...
static __attribute__((unused)) void sm_transition_image(VkCommandBuffer cmd, VkImage img,
...
VkResult sm_upscale_init(SmUpscaleCtx* ctx, const SmUpscaleInitInfo* info) {
    ...
    fprintf(stderr, "[sm_upscale] FP16 shader support: %s\n",
            ctx->supports_fp16 ? "YES" : "NO (falling back to FP32)");

    /* ── Create samplers ── */
    r = sm_create_samplers(ctx);
    if (r != VK_SUCCESS) return r;

    /* ── Shader modules from caller-provided SPIR-V ── */
    r = sm_create_shader_module(ctx->device, ctx->alloc,
    ...
    r = sm_create_descriptors(ctx);
    if (r != VK_SUCCESS) return r;

    /* ── Compute pipelines ── */
    r = sm_create_pipelines(ctx);
    if (r != VK_SUCCESS) return r;
    ...
    for (int i = 0; i < 2; i++) {
        r = sm_create_image_2d(ctx->device, info->phys_device, ctx->alloc,
    ...
    r = sm_upscale_load_weights(ctx, info->weight_file_path);
    if (r != VK_SUCCESS) return r;

    fprintf(stderr, "[sm_upscale] Initialized: max display %ux%u, mode=%s\n",
            ctx->max_w, ctx->max_h,
                             ...
                             const SmUpscaleFrameParams* p)
{
    ...
    float sharpness   = p->sharpness > 0.0f ? p->sharpness
                       : sm_us_env_float("SM_UPSCALE_SHARP", 0.2f);
    float clamp_gamma = p->clamp_gamma > 0.0f ? p->clamp_gamma
                       : sm_us_env_float("SM_UPSCALE_CLAMP_G", 1.0f);
    int debug_mode    = p->debug_mode ? p->debug_mode
                       : sm_us_env_int("SM_UPSCALE_DEBUG", 0);
     ...
     * ═══════════════════════════════════════════════════════════ */
    {
        ...
        int force_mode = sm_us_env_int("SM_UPSCALE_MODE", 0);
...
void sm_upscale_destroy(SmUpscaleCtx* ctx) {
    ...
    /* Images */
    for (int i = 0; i < 2; i++)
        sm_destroy_image(dev, a, ctx->history_img[i], ctx->history_mem[i], ctx->history_view[i]);
    sm_destroy_image(dev, a, ctx->reproj_img, ctx->reproj_mem, ctx->reproj_view);
    sm_destroy_image(dev, a, ctx->confidence_img, ctx->confidence_mem, ctx->confidence_view);
