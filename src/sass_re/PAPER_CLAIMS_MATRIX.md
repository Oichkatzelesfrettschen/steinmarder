...
| C04 | Direct local source/IR on the current toolchain stack does not emit `P2R.B1`, `P2R.B2`, or `P2R.B3`. | bounded-negative | [P2R_FRONTIER_ANALYSIS.md](P2R_FRONTIER_ANALYSIS.md), [P2R_NEXT_PHASE.md](P2R_NEXT_PHASE.md) | This is a strong negative result over the tested CUDA source, PTX, clang, Triton, and `ptxas` sweep space. |
...
| C06 | Simple PTX-level, clang, Triton, and tested `ptxas 11.8/12.6/13.1` sweeps do not unlock direct local `P2R.B*` emission. | bounded-negative | [P2R_NEXT_PHASE.md](P2R_NEXT_PHASE.md), [KNOWLEDGE_SYNTHESIS.md](KNOWLEDGE_SYNTHESIS.md) | This is a bounded statement over the tested basins, not a proof that no older sysroot or deeper IR layer could do it. |
...
| C18 | The best next paper-worthy frontier is motif-sensitive widening from the live `UPLOP3` anchors rather than more generic `P2R` source mutation. | open | [KNOWLEDGE_SYNTHESIS.md](KNOWLEDGE_SYNTHESIS.md), [FRONTIER_ROADMAP.md](FRONTIER_ROADMAP.md) | This is the current forward-looking research hypothesis, not a closed result. |
