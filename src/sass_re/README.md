...
- Recursive probe corpus: 349 `probe_*.cu` files
- Non-skip manifest entries: 347
- Compile-enabled manifest entries: 343
- Manifest runner kinds: 333 `plain`, 2 `texture_surface`, 1 `cp_async_zfill`,
  1 `mbarrier`, 1 `barrier_arrive_wait`, 1 `barrier_coop_groups`,
  1 `cooperative_launch`, 1 `tiling_hierarchical`, 1 `depbar_explicit`,
  1 `optical_flow`, 1 `optix_pipeline`, 1 `optix_callable_pipeline`,
  1 `video_codec`, 1 `cudnn`, and 2 `skip`
- Dedicated validation runners: `runners/dp4a_signedness_runner.cu`,
  `runners/cp_async_zfill_runner.cu`, `runners/texture_surface_runner.cu`,
  `runners/mbarrier_runner.cu`, `runners/barrier_arrive_wait_runner.cu`,
  ...
  `runners/optix_real_pipeline_runner.cu`,
  `runners/optix_callable_pipeline_runner.cu`,
  `runners/ofa_pipeline_runner.cu`, `runners/nvenc_nvdec_pipeline_runner.cu`,
  `runners/cudnn_conv_mining_runner.cu`
...
The current canonical aggregate run is
[`full_recursive_20260320_182500`](results/runs/full_recursive_20260320_182500).
...
src/sass_re/
  ...
  runners/           dedicated host runners for runtime validation
