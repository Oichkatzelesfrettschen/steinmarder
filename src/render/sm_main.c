...
void sm_render_360(const Camera *cam, const char *out_ppm);
...
// -------------------------
static void sm_run_cpu_bvh_baseline(const Camera *cam, int w, int h) {
    printf("[BVH] baseline start...\n");

    // Scene path priority:
    // 1) SM_BASELINE_SCENE env
    // 2) ./scene.txt
    // 3) ./DATA/scene.txt
    const char *scene_path = getenv("SM_BASELINE_SCENE");
    ...
    if (N <= 0) {
        printf("[BVH] baseline: no spheres loaded (set SM_BASELINE_SCENE or provide scene.txt)\n");
        free(tmp);
    ...
    const char* pol = getenv("SM_BVH_POLICY");
    if (pol && pol[0]) {
        bvh_load_policy_csv(pol, root);
    } else {
        printf("[BVH] policy: SM_BVH_POLICY not set\n");
    }
    /* ===== PASS-2 (END) ===== */
...
int main(void)
{
    ...
    // Config (env)
    // -------------------------
    int image_width       = env_int("SM_W", 800);
    int image_height      = env_int("SM_H", 450);
    int samples_per_pixel = env_int("SM_SPP", 64);
    int max_depth         = env_int("SM_DEPTH", 8);
    int thread_count      = env_int("SM_THREADS", 0); // 0 => auto
    int tile_size         = env_int("SM_TILE", 32);
    ...
    if (getenv("SM_NERF_HASHGRID")) {
        float nerf_bounds = 4.0f;
        const char *bounds_s = getenv("SM_NERF_BOUNDS");
        ...
        const char *cam_pos_s = getenv("SM_NERF_CAM");
    ...
    if (getenv("SM_NERF_HASHGRID")) {
        ...
        if (hashgrid_path && occ_path) {
            sm_nerf_init(hashgrid_path, occ_path, (uint32_t)image_width, (uint32_t)image_height);
        }
        ...
        /* Shutdown any NeRF global resources allocated by sm_nerf_init */
        sm_nerf_shutdown();
    ...
    // -------------------------
    sm_neural_denoise_maybe(pixels, image_width, image_height);

    // Optional dump (toggle: SM_DUMP_RGB=1)
    if (getenv("SM_DUMP_RGB")) {
        // if no real implementation, stub below returns 0
        (void)sm_dump_rgb32("output_color.smb", pixels, image_width, image_height);
    }
    ...
    // 360 render
    // -------------------------
    printf("[main] calling sm_render_360...\n");
    sm_render_360(&cam, "output_360.ppm");
    printf("steinmarder 360: wrote output_360.png\n");
    ...
    // -------------------------
    sm_run_cpu_bvh_baseline(&cam, image_width, image_height);

    printf("[main] END\n");
    return 0;
}

// `sm_dump_rgb32` is provided by gbuffer_dump.c; no stub here.
