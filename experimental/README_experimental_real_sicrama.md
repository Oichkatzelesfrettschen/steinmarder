...
Sunulanlar:
- `SM_Ray8`: 8-wide ray packet (SoA)
- `SM_Tri8`: 8-wide triangle packet (SoA, v0 + edges)
- `sm_intersect_ray8_tri1`: 8 ray vs 1 triangle (Möller–Trumbore, AVX2)
- `sm_intersect_ray1_tri8`: 1 ray vs 8 triangles (AVX2), en yakın hit seçer
...
`gcc -O3 -mavx2 -std=c11 experimental/sm_packet.c vec3.c ray.c -o packet_test.exe`

## Entegrasyon Planı (Doğru Sıra)
1) BVH leaf'te triangle listelerini 8'li bloklara düzenle
2) Leaf intersection: `sm_intersect_ray1_tri8` (ray scalar, tri packet)
